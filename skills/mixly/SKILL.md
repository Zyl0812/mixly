...
name: mixly
description: >
  Operate the local mixly CLI music player via shell commands: search Netease/QQ/Bilibili
...
用 shell 命令控制本机 `mixly`（网易云 + QQ + B站 + 本地文件混合歌单播放器，依赖 PATH 上的 mpv）。
...
4. 登录要人扫码：启动 `mixly login --platform netease|qq|bilibili` 后让用户扫终端二维码，不要假装已登录。未登录时 QQ 常拿不到播放链；B站 未登录时搜索会被跳过或提示登录。
5. 搜索输出行 `[平台] 歌名-专辑-歌手  id=X`：播放/加歌单用 `id=` 的值；平台参数固定 `netease` | `qq` | `bilibili` | `local`，不要用中文。
6. 不要并行开多个 play（抢 mpv 管道）；不要把 token 文件内容贴进对话（含 B站 `bilibili_token.json`）。
7. B站 只播放普通 UGC 视频的音频，无歌词属正常；默认不选 B站 版本，只有用户明确要求时才用。

## 命令速查

```powershell
mixly search "关键词" [--platform all|netease|qq|bilibili|local] [--limit 10]
mixly play qq 0039MnYb0qxYhV                        # 平台 + id 精确播
mixly play bilibili BV1xx411c7mD                    # B站：BV 号（视频音频）
...
mixly playlist add "通勤" bilibili BV1xx411c7mD --name "视频音频"
...
mixly login --platform netease|qq|bilibili
mixly logout --platform bilibili                     # 退出登录并删除本机凭证
mixly config prefer qq|netease|bilibili              # 搜索优先侧持久化（默认 qq）
mixly config show
...
- **B站 播放**：先 `mixly login --platform bilibili` 让用户扫码，再 `search --platform bilibili` 或 `play bilibili <BV号/av号/URL>`。
...
4. 使用歌名和歌手搜索并验证：
   ...
   必须选择歌名和歌手都匹配的正式版本，不选择翻唱、伴奏、DJ 或 Live 版本。默认优先正式音乐平台（QQ/网易云）的版本；**只有用户明确要求 B站 版本时**才从 `--platform bilibili` 的结果中选择。
...
- B站 未登录 / 登录过期 → 提示用户 `mixly login --platform bilibili` 扫码；不要读取或粘贴 token。
- B站 风控/地区/权限错误 → 尊重结果，不绕过；建议稍后再试。
